  ...
  try {
    ...
    if (name !== undefined) {
      ...
      const [dupes] = await conn.query(
        'SELECT id FROM bank_accounts WHERE user_id = ? AND id != ? AND LOWER(name) = LOWER(?)',
        [req.userId, acct.id, trimmedName]
      );
      if (dupes.length > 0) {
        await conn.rollback();
        return res.status(400).json({ error: 'You already have a bank account with this name.' });
      }
      await conn.query('UPDATE bank_accounts SET name = ? WHERE id = ?', [trimmedName, acct.id]);
    }
